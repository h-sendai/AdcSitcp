...
static const char* adcsitcpreader_spec[] =
{
    "implementation_id", "AdcSitcpReader",
    "type_name",         "AdcSitcpReader",
    "description",       "AdcSitcpReader component",
    "version",           "1.0",
...
AdcSitcpReader::AdcSitcpReader(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_OutPort("adcsitcpreader_out", m_out_data),
      m_sock(0),
      m_read_byte_size(0),
      m_epfd(-1),
      m_ev_ret(NULL),
      ...
      m_debug(false)
{
    ...
    // Set OutPort buffers
    registerOutPort("adcsitcpreader_out", m_OutPort);

    init_command_port();
    init_state_table();
    set_comp_name("ADCSITCPREADER");
}

AdcSitcpReader::~AdcSitcpReader()
{
}

RTC::ReturnCode_t AdcSitcpReader::onInitialize()
{
    if (m_debug) {
        std::cerr << "AdcSitcpReader::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t AdcSitcpReader::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int AdcSitcpReader::daq_dummy()
{
    return 0;
}

int AdcSitcpReader::daq_configure()
{
    fprintfwt(stderr, "AdcSitcpReader::configure\n");
    //std::cerr << "*** AdcSitcpReader::configure" << std::endl;
...
int AdcSitcpReader::parse_params(::NVList* list)
{
...
int AdcSitcpReader::daq_unconfigure()
{
    fprintfwt(stderr, "AdcSitcpReader::unconfigure\n");
    //std::cerr << "*** AdcSitcpReader::unconfigure" << std::endl;
    m_module_list.clear();

    return 0;
}

int AdcSitcpReader::daq_start()
{
    fprintfwt(stderr, "AdcSitcpReader::start\n");
    //std::cerr << "*** AdcSitcpReader::start" << std::endl;
...
int AdcSitcpReader::daq_stop()
{
    fprintfwt(stderr, "AdcSitcpReader::stop\n");
    //std::cerr << "*** AdcSitcpReader::stop" << std::endl;
...
int AdcSitcpReader::daq_pause()
{
    std::cerr << "*** AdcSitcpReader::pause" << std::endl;

    return 0;
}

int AdcSitcpReader::daq_resume()
{
    std::cerr << "*** AdcSitcpReader::resume" << std::endl;

    return 0;
}

int AdcSitcpReader::set_data(unsigned char *read_buf, unsigned int data_byte_size)
{
...
int AdcSitcpReader::write_OutPort()
{
...
int AdcSitcpReader::get_data_byte_size(unsigned char *buf, int len)
{
    int *l;
    int rv;
    l = (int *) &buf[4];
...
int AdcSitcpReader::daq_run()
{
    if (m_debug) {
        std::cerr << "*** AdcSitcpReader::run" << std::endl;
    }
...
extern "C"
{
    void AdcSitcpReaderInit(RTC::Manager* manager)
    {
        RTC::Properties profile(adcsitcpreader_spec);
        manager->registerFactory(profile,
                    RTC::Create<AdcSitcpReader>,
                    RTC::Delete<AdcSitcpReader>);
    }
};
